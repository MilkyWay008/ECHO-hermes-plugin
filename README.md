...
|---------|------|---------|
| 2.3.1 | 2026-07-18 | Fixed: one-shot subprocess uses `sys.executable` (not hardcoded `"python"`). Compare script now uses word-level matching to prevent false yellow verdicts from rephrasing. Updated verdict instructions with explicit "STOP. Read files first" format. Changed SOUL.md injection to structured format. |
